...
def analyzedata(text):
    ...
    with h5py.File('data.h5', 'w') as hdf:
        ...
        for column in dataIn.columns:
            #other
            dataIn[column] =  dataIn[column].rolling(window=2).mean()
            print(dataIn)
        ...
        for i in range(count_segmentsIn):
            ...
            featsIn.loc[i] = [featuresIn[j][3] for j in range(len(featuresIn))]
        ...
        for i in range(count_segments):
            ...
            # fig11, ax11 = plt.subplots(figsize=(10, 10), layout="constrained")
            # plt.title('Preprocessed Data [X Axis]')
            # plt.xlabel('Time (s)', fontsize=15)
            # plt.ylabel('Acceleration', fontsize=15)
            # ax11.plot(df[0], df[1])
            #
            # fig11, ax11 = plt.subplots(figsize=(10, 10), layout="constrained")
            # plt.title('Preprocessed Data [Y Axis]')
            # plt.xlabel('Time (s)', fontsize=15)
            # plt.ylabel('Acceleration', fontsize=15)
            # ax11.plot(df[0], df[2])
            #
            # fig11, ax11 = plt.subplots(figsize=(10, 10), layout="constrained")
            # plt.title('Preprocessed Data [Z Axis]')
            # plt.xlabel('Time (s)', fontsize=15)
            # plt.ylabel('Acceleration ', fontsize=15)
            # ax11.plot(df[0], df[3])
            #
...
#
#     # Plot each acceleration on a separate subplot
#     axs.plot(File.iloc[:, 10])
